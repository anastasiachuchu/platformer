...
WALK = [(f'{dir}\\png\\Sprite_me1.png'), #анимация хотьбы
...
SINK = [(f'{dir}\\png\\Sprite_sink_0.png'), #анимация персонаж тонет
...
class Player(pg.sprite.Sprite):
    ...
    def __init__(self,sprite_group):
        ...
        self.rect = Rect((self.x+20, self.y, 30, 100))
        self.right = False
        self.left = False
        ...
        self.animation_stay = pyganim.PygAnimation(STAY) #реализации анимации персонаж стоит с помощью pyganim
        self.animation_stay.play()
                
        self.animation_jump= pyganim.PygAnimation(JUMP) #реализации анимации прыжка с помощью pyganim
        self.animation_jump.play()

        animation_s = [] #реализации анимации персонаж тонет с помощью pyganim
        for anim in SINK:
    ...
    def update(self,platforms, len_level): #метод изменения персонажа в пространстве
        ...
        global sink_number
        if not self.stop_move: #если движение персонажа не приостановлено, то он может изменять положение в пространстве
                    ...
                    if self.jump_power >= -9:
                        if self. jump_power > 0: #поднимает персонажа
                            ...
                            if self.right_jump: #смещение при прыжке вправо
                                    self.rect.x += 10
                            elif self.left_jump: #смещение при прыжке влево
                                    self.rect.x -= 6
        ...
        if not self.on_ground and not self.up: 
            if not self.loss:
                ...
                if sink_number == 0:
                    ...
                    self.stop_move = True
                if sink_number <= 59:
                    self.animation_sink.blitFrameNum(sink_number//15, self.image, (0, 0))
                    sink_number += 1
                else:
                    self.lives -= 1
                    sink_number = 0
    ...
    def collide_platform(self, platforms : list):
        ...
        for platform in platforms:
                if sprite.collide_rect(self, platform):
                    ...
                    self.state = platform.rect.x #сохранение персонажа на x коорденате последнего пройденного островка
